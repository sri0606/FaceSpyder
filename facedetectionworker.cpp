...
FaceDetectionWorker::FaceDetectionWorker(FaceRecognition* faceRecognition): mFaceRecognition(faceRecognition)
{
}
...
void FaceDetectionWorker::processVideo()
{
    ...
    while (true && !mShouldStop) {
        ...
        // Loop through the detected faces and draw bounding boxes
        for (const cv::Rect& face : faces) {
            cv::rectangle(currentMatFrame, face, cv::Scalar(0, 0, 255), 2);  // Draw a red rectangle around the face
        }
    }
