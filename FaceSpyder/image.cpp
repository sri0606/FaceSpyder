 ...
 * Image Constructor
 */
Image::Image(const QString &filename, FaceRecognition *facrec)
...
void Image::OnPaint(std::shared_ptr<QPainter> painter)
{
    ...
    // Rescale the image
    QImage scaledImage = mImage.scaled(newWidth,
                                       newHeight,
                                       Qt::KeepAspectRatio,
    ...
    QPixmap scaledDetectedPixmap = mImageDetected.scaled(newWidth,
                                                         newHeight,
                                                         Qt::KeepAspectRatio,
                                                         Qt::SmoothTransformation);
